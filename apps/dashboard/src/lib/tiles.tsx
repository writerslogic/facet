...
export const CHART_CYCLE: SizeKey[] = [
	"md",
	"lg",
	"short",
	"wide",
...
function Stat({ label, value }: { label: string; value: string }): ReactNode {
	return (
		<div className="flex items-baseline justify-between gap-2 border-white/10 border-b py-2 last:border-0">
			...
			<span className="tabular font-semibold text-neutral-900 text-sm">
				{value}
			</span>
		</div>
	);
...
export const TILE_REGISTRY: Record<string, TileDef> = {
	...
	visitors: {
		...
		render: (ctx, expanded) => (
			<KpiTile
				...
				viz="gauge"
				gaugeRatio={
					ctx.summary.pageviews > 0
						? ctx.summary.visitors / ctx.summary.pageviews
						: 0
				}
				gaugeLabel="of views"
				expanded={expanded}
	...
	flow: {
		...
		expandable: true,
		render: (ctx, expanded) => (
			<FlowTile cells={ctx.flowCells} dark expanded={expanded} />
		),
	},
	pages: {
		id: "pages",
