// C.10: multi-site profiles. Switching the active profile changes the site and must not show the
...
describe("multi-site profiles", () => {
	...
	it("surfaces the auth banner for a profile with an invalid key", async () => {
		localStorage.setItem(
			"facet.profiles",
			JSON.stringify([
				{ id: "a", label: "Site A", siteId: SITE_A, apiKey: "clk_bad" },
			]),
		);
		localStorage.setItem("facet.activeProfile", "a");
		vi.stubGlobal(
	...
	it("persists profiles across a remount", async () => {
		...
		await waitFor(() =>
			expect(screen.getByLabelText("Active site")).toBeInTheDocument(),
		);
		unmount();
		...
		expect(
			screen.getByRole("option", { name: "Site B" }),
		).toBeInTheDocument();
	});
});
