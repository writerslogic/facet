...
async function build(
	n: number,
): Promise<{ nodes: Uint8Array[]; leafIndices: number[] }> {
	const nodes: Uint8Array[] = [];
	const leafIndices: number[] = [];
	for (let k = 0; k < n; k++) {
		leafIndices.push(
			await addLeafHash(nodes, await leafHash(`rollup-${k}`)),
		);
	}
...
describe("index math (draft reference values)", () => {
	it("index_height matches the reference MMR layout", () => {
		...
		expect([0, 1, 2, 3, 4, 5, 6].map(indexHeight)).toEqual([
			0, 0, 1, 0, 0, 1, 2,
		]);
	});
...
describe("MMR inclusion", () => {
	...
	it("a proof does not verify against a different tree root", async () => {
		...
		const proof = proveInclusion(
			a.nodes,
			a.leafIndices[2] as number,
			a.nodes.length,
		);
		const rootB = await baggedRoot(
			b.nodes.length,
			accumulatorHashes(b.nodes, b.nodes.length),
		);
...
describe("MMR consistency", () => {
	it("proves an earlier checkpoint is a prefix of a later one", async () => {
		...
		const nodes: Uint8Array[] = [];
		for (let k = 0; k < 5; k++)
			await addLeafHash(nodes, await leafHash(`r-${k}`));
		const sizeFrom = nodes.length;
		const rootFrom = await baggedRoot(
			sizeFrom,
			accumulatorHashes(nodes, sizeFrom),
		);

		for (let k = 5; k < 12; k++)
			await addLeafHash(nodes, await leafHash(`r-${k}`));
		const sizeTo = nodes.length;
		const rootTo = await baggedRoot(
			sizeTo,
			accumulatorHashes(nodes, sizeTo),
		);
