...
import { deriveVisitorHash, getScopedSalt, windowEndMs, windowKey } from '../src/lib/identity.js';
...
async function storedHashes(e: typeof env): Promise<string[]> {
	const rows = await e.DB.prepare('SELECT visitor_hash FROM events WHERE site_id = ?')
		.bind(SITE)
...
describe('ingestEvent under the identity spectrum', () => {
	it('a zero-config site writes the legacy Tier-0 hash (no-op)', async () => {
		...
		const legacy = await visitorHash(IP, UA, await getDailySalt(env, dayKey(NOW), NOW), SITE);
	...
	describe('elevated site', () => {
		...
		it('writes the windowed hash when a valid consent record exists', async () => {
			...
			const vh = await deriveVisitorHash('pseudonymous', { ip: IP, ua: UA }, salt, SITE);
