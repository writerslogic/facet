// T034: `countless init --dry-run` writes a wrangler.jsonc (with the given name + db) and a
...
describe('runInit', () => {
	it('scaffolds wrangler.jsonc and .dev.vars under --dry-run', async () => {
		const dir = mkdtempSync(join(tmpdir(), 'countless-init-'));
		...
		expect(wrangler).toContain('"database_name": "countless"');
